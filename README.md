...
<section id="top" class="max-w-6xl mx-auto px-4 py-8">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
    ...
    <!-- Replaced student image with Web3 abstract SVG -->
...
<!-- TESTIMONIALS replaced with Web3 avatars -->
<section class="py-12 px-4">
  <div class="max-w-5xl mx-auto">
    <h3 class="text-2xl font-bold neon-text text-center mb-6">Student Voices</h3>
    <div class="grid md:grid-cols-3 gap-4">
      <div class="glass p-4 rounded-lg float-card text-center">
        <svg viewBox="0 0 100 100" class="mx-auto mb-3 w-20 h-20">
          <circle cx="50" cy="50" r="45" fill="url(#grad)" opacity="0.25"/>
          <circle cx="50" cy="50" r="20" fill="url(#grad)"/>
        </svg>
        <p class="text-gray-300 italic">“Zep Swipe gave me ZAC for learning Web3 basics.”</p>
        <div class="mt-2 font-semibold text-pink-300">— Ama, Ghana</div>
      </div>

      <div class="glass p-4 rounded-lg float-card text-center">
        <svg viewBox="0 0 100 100" class="mx-auto mb-3 w-20 h-20">
          <rect x="10" y="10" width="80" height="80" rx="20" fill="url(#grad)" opacity="0.25"/>
          <path d="M20 50 H80" stroke="url(#grad)" stroke-width="3"/>
        </svg>
        <p class="text-gray-300 italic">“It feels like a mini-game with real rewards.”</p>
        <div class="mt-2 font-semibold text-pink-300">— Sanele, South Africa</div>
      </div>

      <div class="glass p-4 rounded-lg float-card text-center">
        <svg viewBox="0 0 100 100" class="mx-auto mb-3 w-20 h-20">
          <polygon points="50,10 90,90 10,90" fill="url(#grad)" opacity="0.25"/>
          <circle cx="50" cy="50" r="15" fill="url(#grad)"/>
        </svg>
        <p class="text-gray-300 italic">“Love that I can pay at cafés with $ZAC.”</p>
        <div class="mt-2 font-semibold text-pink-300">— Kofi, Kenya</div>
      </div>
    </div>
  </div>
</section>
...
function loop(){
x.clearRect(0,0,W,H);
p.forEach(a=>{a.x+=a.vx;a.y+=a.vy;if(a.x<0)a.x=W;if(a.x>W)a.x=0;if(a.y<0)a.y=H;if(a.y>H)a.y=0;x.fillStyle='rgba(0,240,255,0.12)';x.beginPath();x.arc(a.x,a.y,2,0,6.28);x.fill();});
requestAnimationFrame(loop);
}
loop();

document.getElementById('connectHeader').onclick=()=>alert('Mock Connect Wallet. Integrate WalletConnect in production.');
document.getElementById('connectHero').onclick=()=>alert('Mock Connect Wallet. Integrate WalletConnect in production.');
</script>
</body>
</html>